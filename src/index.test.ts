...
describe("pointToTileFraction", () => {
  ...
  ])("should find tile when %s", (_label, lng, lat, z, expectedTile) => {
...
describe("tilesEqual", () => {
  ...
  ])("should evaluate when %s", (_label, a, b, expectedResult) => {
...
describe("hasTile", () => {
  ...
  ])("should calculate corrently when %s", (_label, tiles, tile, expectedResult) => {
...
describe("getSiblings", () => {
  ...
  ])("should get siblings when %s", (_label, tile, expectedSiblings) => {
...
describe("getParent", () => {
  ...
  ])("should get parent when %s", (_label, tile, expectedParent) => {
...
describe("getChildren", () => {
  ...
  ])("should get children when %s", (_label, tile, expectedChildren) => {
...
describe("tileToQuadkey", () => {
  test.each(quadKeyPairs.map(({ quadkey, tile }) => [tile, quadkey]))(
    "should get quadkey when %j",
    (tile, expectedQuadkey) => {
...
describe("quadkeyToTile", () => {
  test.each(quadKeyPairs.map(({ quadkey, tile }) => [quadkey, tile]))(
    "should get tile when %s",
    (quadkey, expectedTile) => {
...
describe("tileToBBox", () => {
  ...
  ])("should find bbox when %s", (_label, tile, expectedBbox) => {
...
describe("bboxToTiles", () => {
  ...
  test.each<[string, number[], number, number[][]]>([
    ...
    [
      "bbox intersects many",
      [-118.125, -39.375, -106.875, -28.125],
      4,
      [
        [5, 10, 4],
        [5, 11, 4],
  ...
  ])("should find tiles when %s", (_label, bbox, z, expectedTiles) => {
...
describe("bboxesIntersect", () => {
  ...
  ])("should be truthy when %s", (_label, a, b) => {
  ...
  ])("should falsey when %s", (_label, a, b) => {
    const output = tileService.bboxesIntersect(a, b);
...
describe("intersectBboxes", () => {
  ...
  ])("should intersect when %s", (_label, a, b, expectedBbox) => {
  ...
  test.each<[string, number[], number[]]>([
    ...
    ["a top b", [0, 0, 1, 1], [0, 2, 1, 3]],
    ["a bottom b", [0, 2, 1, 3], [0, 0, 1, 1]]
  ])("should return intersection of undefined when %s", (_label, a, b) => {
    const output = tileService.intersectBboxes(a, b);
